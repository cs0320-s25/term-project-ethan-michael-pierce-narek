...
//    p.remaining = req.queryParams("remaining");
  ...
  public static void validateCourseExistence(Params p,
      Set<String> validCodes,
      List<String> errors) {

    List<String> bad = p.necessaryCourses.stream()
    ...
    if (!bad.isEmpty()) {
      if (bad.size() == 1) {
        errors.add(String.format(
            "Course \"%s\" doesn’t seem to exist",
            bad.get(0)
        ));
      } else {
        errors.add(String.format(
            "These courses don’t seem to exist: %s",
            String.join(", ", bad)
        ));
      }
    }
